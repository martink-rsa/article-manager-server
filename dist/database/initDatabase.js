...
exports.populateDatabase = exports.initDatabase = exports.dropTables = void 0;
/**
...
function initDatabase(connection) {
    ...
    connection.query('ALTER TABLE Articles ADD FOREIGN KEY (employeeID) REFERENCES Employees (id);', function (err, result) {
        if (err)
            throw err;
...
function populateDatabase(connection) {
    ...
    // Countries
    connection.query(`INSERT INTO Countries (name)
    VALUES ('South Africa'), ('United Kingdom'), ('United States');`, function (err, result) {
        if (err)
            throw err;
        console.log('Data inserted into: Countries');
    });
    // Positions
    connection.query(`INSERT INTO Positions (position)
    VALUES ('Director'), ('Customer Service'), ('Developer');`, function (err, result) {
        if (err)
            throw err;
    ...
    VALUES ('HR'), ('Marketing'), ('Web Development');`, function (err, result) {
        if (err)
            throw err;
        console.log('Data inserted into: Departments');
    });
    // Employees
    connection.query(`INSERT INTO Employees (firstName, lastName, department, position, country, email, password, articles )
    VALUES ('First Name', 'Last Name', 1, 1, 1, 'test@test.com', 'password123', NULL);`, function (err, result) {
        if (err)
            throw err;
        console.log('Data inserted into: Employees');
    });
    // Articles
    connection.query(`INSERT INTO Articles (title, description, url, employeeId, usersDisliked, usersLiked)
    VALUES ('Page title', 'This is the description', 'www.google.com', 2, 3, 6);`, function (err, result) {
        if (err)
            throw err;
